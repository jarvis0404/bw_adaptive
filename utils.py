...
def save_nets(job_name, nets, epoch):
    ...
    if not os.path.exists(dirpath):
        print('Creating model directory: {}'.format(dirpath))
        os.makedirs(dirpath, exist_ok=True)

    path = os.path.join(dirpath, f"{job_name}.pth")
...
def load_weights(job_name, nets, device = torch.device('cuda'), path = None):
    ...
    if path is not None:
        ...
        return cp.get('epoch', None)

    # Otherwise, try to find the most recent dated subdirectory containing the job file
    models_root = 'models'
    candidate = None
    if os.path.exists(models_root):
        # list subdirectories under models that look like dates (YYYYmmdd) or any directories
        entries = [os.path.join(models_root, d) for d in os.listdir(models_root) if os.path.isdir(os.path.join(models_root, d))]
        # sort descending by name (so YYYYMMDD latest comes first)
        entries.sort(reverse=True)
        for d in entries:
            p = os.path.join(d, f"{job_name}.pth")
            if os.path.exists(p):
                candidate = p
                break

    # fallback to top-level models/<job_name>.pth
    if candidate is None:
        fallback = os.path.join('models', f"{job_name}.pth")
        if os.path.exists(fallback):
            candidate = fallback

    if candidate is None:
        raise FileNotFoundError(f'Could not find checkpoint for {job_name} in models/ or subdirectories')

    cp = torch.load(candidate, map_location = device)
    nets.load_state_dict(cp['jscc_model'], strict=False)
    return cp.get('epoch', None)
