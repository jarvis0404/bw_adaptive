...
class Swin_JSCC(nn.Module):
    def __init__(self,  args, enc, dec):
        ...
        self.linear_embed = nn.Linear(3, args.n_adapt_embed).to(args.device)
    ...
    def gen_embedding(self, bw, snr = None, plr = 0.0):
        ...
        adapt_embed = torch.from_numpy(np.array([snr, bw, plr])).float().to(self.device)
    ...
    def forward(self, img, bw, snr = None, plr = 0.0):
        ...
        if self.args.channel_mode == 'rayleigh':
            h_sd = self.complex_sig(noise_shape, self.device)
            h_sd = self.pwr_normalize(h_sd)
            y_sd = h_sd * sig_s + np.sqrt(noise_sd_pwr) * noise_sd
            # perfect CSI equalization (Zero-Forcing with robust regularization)
            h_sd_conj = torch.conj(h_sd)
            h_mag_sq = h_sd.real**2 + h_sd.imag**2
            y_sd = y_sd * h_sd_conj / (h_mag_sq + 1e-8)
        else:  # default AWGN
            y_sd = sig_s + np.sqrt(noise_sd_pwr) * noise_sd

        # Packet Loss Simulation
        if plr > 0:
            # Generate mask for packet loss (drop entire patches or symbols?)
            # Since x is (B, H_W_, n_feat), let's drop patches (H_W_ dimension)
            # y_sd is complex (B, H_W_*n_feat/2) or similar
            # Let's reshape y_sd to match patches
            # x shape: B, H_W_, n_feat
            # sig_s shape: B, H_W_*n_feat/2
            
            # Reshape y_sd back to patch structure to apply patch-wise loss
            # Note: y_sd is currently flat complex sequence (B, N_symbols)
            # N_symbols = H_W_ * n_feat / 2
            
            # We want to drop patches. Each patch corresponds to n_feat/2 complex symbols.
            symbols_per_patch = n_feat // 2
            
            y_sd_reshaped = y_sd.view(B, H_W_, symbols_per_patch)
            
            # Mask shape: (B, H_W_, 1)
            # Bernouli distribution: 1 with prob (1-plr), 0 with prob plr
            keep_prob = 1.0 - plr
            loss_mask = torch.bernoulli(torch.full((B, H_W_, 1), keep_prob, device=self.device))
            
            y_sd_reshaped = y_sd_reshaped * loss_mask
            y_sd = y_sd_reshaped.view(B, -1)
